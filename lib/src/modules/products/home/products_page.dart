...
class _ProductsPageState extends State<ProductsPage> with Loader, Messages {
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      statusDisposer = reaction((status) => controller.status, (status) async {
        switch (status) {
          ...
          case ProductStateStatus.addOrUpdateProduct:
            ...
            final productModelSelected = controller.productModelSelected;
            if(productModelSelected != null){
              uri += '?id=${productModelSelected.id}';
            }
            ...
            child: Observer(builder: (_) {
              return GridView.builder(
                ...
                itemBuilder: (context, index) {
                  return ProductItem(model: controller.products[index]);
                },
              );
            }),
          )
